...
- 关闭 LLM 主动查询工具时，插件会匹配当前消息及最近六条会话消息，作为代词追问的兜底。
- 当前发言者已绑定平台身份时，即使消息未出现姓名或别名，也会选择对应人物。
- 人物资料、关系方向、类型、强度、状态和描述会加入本次 LLM 请求；管理员备注、平台 ID、昵称次数和关系证据不会注入。
- “关系注入位置”默认为“系统提示词”，也可改为“用户消息附加内容”；后者作为本轮临时内容块发送。
- 注入内容不会写入用户的聊天记录。每次实际注入都会输出 `[PersonalNetwork] Injected relationship context` 日志。
