...
- 启用 LLM 主动查询工具时，自动注入只匹配当前消息中的人物姓名和别名；代词追问由模型按需调用查询工具，避免连续多轮重复注入关系文本。
- 关闭 LLM 主动查询工具时，可通过“启用最近消息兜底”和“最近消息兜底数量”控制是否回看历史及回看的消息数量，默认回看最近 6 条。
- 当前发言者已绑定平台身份时，即使消息未出现姓名或别名，也会选择对应人物。
- 人物资料、关系方向、类型、强度、状态和描述会加入本次 LLM 请求；管理员备注、平台 ID、昵称次数和关系证据不会注入。
- “关系注入位置”默认为“系统提示词”，也可选择“用户消息附加内容”或“持久化伪历史消息”。持久化模式会在当前会话中保存至多一条隐藏关系消息，后续查询结果替换旧消息，不会重复追加；切换到其他位置后旧消息会在下一轮自动移除。
- 持久化伪历史消息可以避免同一关系反复调用查询工具，但模型每轮仍需读取这条消息，因此仍会占用必要的输入 token。
- 系统提示词和用户消息附加内容不会写入会话历史；持久化模式会写入一条隐藏系统消息，但不会伪造用户或助手发言。每次实际注入都会输出 `[PersonalNetwork] Injected relationship context` 日志。
